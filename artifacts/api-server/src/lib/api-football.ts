...
// Sezon — ücretsiz planda 2022-2024 destekleniyor
const SEASON = 2024;

// Desteklenen lig ID'leri — API-Football lig id'leri
...
// Pozisyon dönüştürücü: API adını oyundaki kısa forma çevir
...
async function apiFetch<T>(path: string): Promise<T> {
  if (!API_KEY) throw new Error("API_FOOTBALL_KEY ortam değişkeni tanımlı değil");

  const url = `${BASE_URL}${path}`;
  logger.info({ url }, "API-Football isteği");
